...
import { ensureCommissionDueReminders } from "@/lib/commissionReminders";
...
export async function GET(req: NextRequest) {
  ...
  try {
    await connectDB();
    if (role === "admin" || role === "staff") await ensureCommissionDueReminders();
...
export async function PATCH(req: NextRequest) {
  ...
  const update =
    action === "toast_shown"
      ? { $addToSet: { toast_shown_by: userId }, $set: { updated_at: now } }
      : { $addToSet: { read_by: userId }, $set: { updated_at: now } };
  ...
  try {
    ...
    const store = await readStore();
    const field = action === "toast_shown" ? "toast_shown_by" : "read_by";
    store.notifications = (store.notifications || []).map((notification: any) =>
      recordId(notification) === notificationId
        ? {
            ...notification,
            [field]: Array.from(new Set([...(notification[field] || []), userId])),
            updated_at: now,
          }
        : notification
    );
