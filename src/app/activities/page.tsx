...
import { Clock, Filter, Heart, MapPin, Search, Star } from "lucide-react";
    ...
    <main className="min-h-screen bg-white text-[#10284a]">
      ...
      <section className="border-b border-slate-200 bg-[#f6f8fb]">
        <div className="mx-auto max-w-[1530px] px-4 py-12 sm:px-6 lg:px-8">
          <p className="text-sm font-black uppercase tracking-widest text-[#0875e1]">Kerala activities</p>
      ...
      <section className="mx-auto max-w-[1530px] px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-7 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filtered.map((activity) => (
            <Link key={activity.id} href={`/activities/${activity.slug}`} className="group rounded-lg border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
              <div className="relative overflow-hidden rounded-t-lg">
                <img src={activity.image} alt={activity.title} className="h-56 w-full object-cover transition duration-500 group-hover:scale-105" />
                <button className="absolute right-3 top-3 flex h-11 w-11 items-center justify-center rounded-full bg-white shadow-lg" aria-label="Save activity">
                  <Heart className="h-6 w-6" />
                </button>
              </div>
              <div className="p-5">
                <p className="text-xs font-black uppercase tracking-widest text-[#0875e1]">{activity.category}</p>
                <h2 className="mt-2 text-2xl font-black leading-tight">{activity.title}</h2>
                <div className="mt-3 flex flex-wrap gap-3 text-sm font-semibold text-slate-600">
                  <span className="inline-flex items-center gap-1"><MapPin className="h-4 w-4" /> {activity.location}</span>
                  <span className="inline-flex items-center gap-1"><Clock className="h-4 w-4" /> {activity.duration}</span>
                </div>
                <p className="mt-3 line-clamp-2 text-sm font-medium leading-6 text-slate-600">{activity.description}</p>
